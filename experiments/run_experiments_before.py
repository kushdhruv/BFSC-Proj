...
RESULTS_DIR = os.path.join(os.path.dirname(__file__), '..', 'results_before')
PLOTS_DIR = os.path.join(os.path.dirname(__file__), '..', 'plots_before')
...
def train_rl_agent(
    ...
    simulator = BlockchainSimulator(
        ...
        asymmetric_delay=False,  # Before: no asymmetric
        partition_prob=0.0,      # Before: no partitions
        propagation_variance=0.0, # Before: no variance
        n_nodes=30,              # Before: small network
    )
    training_history = []
...
def run_mode(alpha, delay_intensity, target_fraction, mode, runs=5, episode_blocks=600, fixed_strategy=None, lambda_penalty=0.001, n_nodes=30, asymmetric_delay=False, attacker_delay_multiplier=0.1, partition_prob=0.0, propagation_variance=0.0):
    results = []
    agent = None
    training_history = []
    if mode == 'C':
...
def aggregate_experiments():
    ...
    raw_results.to_csv(os.path.join(RESULTS_DIR, 'sdm_raw_results_before.csv'), index=False)
    summary_results.to_csv(os.path.join(RESULTS_DIR, 'sdm_summary_results_before.csv'), index=False)
...
def plot_results(raw_results, summary_results):
    ...
    for mode in sd['mode'].unique():
        ...
        ax.set_title(f'REV vs alpha (Before Changes - {mode})')
        ax.set_xlabel('alpha')
        ax.set_ylabel('REV')
        ax.legend()
        ax.grid(True)
        fig.savefig(os.path.join(PLOTS_DIR, f'rev_vs_alpha_before_{mode}.png'))
        plt.close(fig)

if __name__ == '__main__':
    raw_results, summary_results = aggregate_experiments()
    plot_results(raw_results, summary_results)
    print('Before results saved to', RESULTS_DIR)
